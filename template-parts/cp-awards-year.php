...
$cpay_year = ( '' !== get_sub_field('cpay_year') ) ? get_sub_field('cpay_year') : null;
$cpay_awards = ( '' !== get_sub_field('cpay_awards') ) ? get_sub_field('cpay_awards') : null;
?>

<section class="panel-layout awards-year-panel ">
	<?php if ( $cpay_year ) {  ?>
		<h2 class="heading-2"> <?php echo esc_html( $cpay_year ); ?>  </h2>
	<?php } ?>
	<?php
	if ( $cpay_awards ) {
	?>
		<div class="container">
			<?php
				foreach( $cpay_awards as $key =>  $cpay_award ){
			...
			?>
				...
				<div class="column-inner">
					<?php
						if ($ay_image) {
							$img = wp_get_attachment_image( $ay_image, 'thumb_255', false, array('class'=>'image-panel-img')  );?>
